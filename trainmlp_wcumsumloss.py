...
wandb.init(project='loss_cumsum_mse-fusion_na-aritificial')
...
class LitAutoEncoder(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        num_voxels = voxel_observations.shape[0]
        ground_truth_cumsum = torch.cumsum(voxel_observations, dim=1)

        oxc = torch.arange(1,num_obs+1).unsqueeze(1).repeat(1,num_classes).to(device)
        divide = oxc.unsqueeze(0).repeat(num_voxels,1,1)
        oxc_encoded = torch.arange(1,num_obs+1).unsqueeze(1).repeat(1,encoded_dim).to(device)
        divide_encoded = oxc_encoded.unsqueeze(0).repeat(num_voxels,1,1)

        ground_truth = ground_truth_cumsum/divide
        ...
        encoded_vectors = encoded_vectors.view(-1, num_obs, encoded_dim)
        encoded_vectors_cumsum = torch.cumsum(encoded_vectors, dim=1)
        fused_encoded_vectors = encoded_vectors_cumsum/divide_encoded
        decodeinput = fused_encoded_vectors.view(-1, encoded_dim)
...
# train model
trainer = L.Trainer(default_root_dir='./checkpoints', max_epochs=1000, logger=wandb_logger, callbacks=[ModelCheckpoint(dirpath='./checkpoints',filename='encoded_dim_2-cumsum_mseloss-na-{epoch}-{val_loss:.5f}',monitor="val_loss", mode="min", save_top_k=2), EarlyStopping(monitor="val_loss", mode="min", patience=50, min_delta=0.0)])
