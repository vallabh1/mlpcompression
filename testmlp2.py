...
# Create encoder, decoder, and fusion network instances
...
optimizer = optim.Adam(list(encoder.parameters()) + list(decoder.parameters()) + list(fusion_network.parameters()), lr=0.0001)
...
for epoch in range(num_epochs):
    ...
    for voxel_idx in range(num_voxels):
        ...
        ground_truth = torch.mean(voxel_observations, dim=0)
       
        # Encode the first observation
        fused_encoded_vector = encoder(voxel_observations[0].unsqueeze(0))
       
        for i in range(1, num_observations):
            current_encoded_vector = encoder(voxel_observations[i].unsqueeze(0))
            # Concatenate the current encoded vector with the fused encoded vector
            fusion_input = torch.cat((current_encoded_vector, fused_encoded_vector), dim=1)
            # Update the fused encoded vector using the fusion network
            fused_encoded_vector = fusion_network(fusion_input)
       
        # Decode the fused vector back to the original probability space
...
torch.save(encoder.state_dict(), 'encoderwkl.pth')
torch.save(decoder.state_dict(), 'decoderwkl.pth')
torch.save(fusion_network.state_dict(), 'fusion_networkwkl.pth')