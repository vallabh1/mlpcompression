...
wandb.init(project="encoded_6-loss_kldiv-fusion_na-aritificial")
...
# train model
trainer = L.Trainer(default_root_dir='./checkpoints', max_epochs=200, logger=wandb_logger, callbacks=[ModelCheckpoint(dirpath='./checkpoints',filename='encoded_dim_6-kldivloss-na-{epoch}-{val_loss:.5f}',monitor="val_loss", mode="min", save_top_k=2), EarlyStopping(monitor="val_loss", mode="min", patience=25, min_delta=0.0)])
