...
class Calibration_calc_3D:
    def __init__(self, tiers=torch.arange(11) / 10, no_void=False, one_hot=True):
        self.tiers = tiers
        self.total_bin_members = torch.zeros(len(tiers) - 1, device=device)
        self.correct_bin_members = torch.zeros(len(tiers) - 1, device=device)
        self.total_bin_confidence = torch.zeros(len(tiers) - 1, device=device)
    ...
    def update_bins(self, semantic_label, semantic_label_gt):
        if self.no_void:
            if self.one_hot:
                gt_labels = torch.argmax(semantic_label_gt, dim=1)
            else:
                gt_labels = semantic_label_gt
            mask = gt_labels != 0
            semantic_label_gt = semantic_label_gt[mask]
            semantic_label = semantic_label[mask]

        max_conf, pred = torch.max(semantic_label, dim=1)
        if self.one_hot:
            comparison_sheet = torch.argmax(semantic_label_gt, dim=1) == pred
        else:
            comparison_sheet = semantic_label_gt == pred

        for i in range(len(self.tiers) - 1):
            if self.tiers[i + 1] != 1.0:
                conf_mask_tier = (max_conf >= self.tiers[i]) & (max_conf < self.tiers[i + 1])
            else:
                conf_mask_tier = (max_conf >= self.tiers[i]) & (max_conf <= self.tiers[i + 1])

            self.total_bin_members[i] += conf_mask_tier.sum().float()
            self.correct_bin_members[i] += comparison_sheet[conf_mask_tier].sum().float()
            self.total_bin_confidence[i] += max_conf[conf_mask_tier].sum().float()
    ...
    def return_calibration_results(self):
        ...
        return acc, conf, self.tiers[1:]

    def get_ECE(self):
        if torch.all(self.total_bin_members == 0):
            return torch.tensor(float('nan'), device=device)
        else:
            acc, conf = self.return_calibration_results()[:2]
            share = torch.nan_to_num(self.total_bin_members / self.total_bin_members.sum(), nan=0.0)
            return (share * torch.nan_to_num(torch.abs(acc - conf), nan=0.0)).sum()


class mECE_Calibration_calc_3D:
    def __init__(self, tiers=torch.arange(11) / 10, no_void=False, one_hot=True, n_classes=21):
        self.tiers = tiers
        self.no_void = no_void
        self.one_hot = one_hot
    ...
    def update_bins(self, semantic_label, semantic_label_gt):
        if self.one_hot:
            map_gt = torch.argmax(semantic_label_gt, dim=1)
        else:
    ...
    def get_mECE(self):
        ...
        ECEs = torch.tensor(ECEs, device=device)
        ECEs[~torch.isfinite(ECEs)] = 1.0
        return torch.mean(ECEs)
...
class LitAutoEncoder(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        cali.update_bins((output_vector_encoded_fusion), gt)
        mece = cali.get_mECE()

        loss = lossf(output_vector_encoded_fusion, ground_truth) + mece
    ...
    def validation_step(self, batch, batch_idx):
        ...
        val_loss = lossf(output_vector_encoded_fusion, ground_truth) + mece
    ...
    def test_step(self, batch, batch_idx):
        ...
        test_loss = lossf(output_vector_encoded_fusion, ground_truth) + mece
